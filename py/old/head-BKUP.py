...
# not invented here
import codecs, base64, os, sys, random, datetime, time, string, shutil, requests, traceback, io, turtle;
...
def ib(custom_message=''):
    ...
    message = message_a + nl + message_b + 'stacktrace in ' + tib.show(stop=True, units='msec') + ' msec' + custom_message + nl 
...
def header(prnt=None):
    if prnt:
        ...
        w('generation in',t1.show(stop=True,units='msec',cut=9),'msec',nl)
##        w(printAttribLn())
